...
import dev.supachain.robot.provider.models.AnthropicMessage.*
...
// context receiver
private fun List<ToolConfig>.asAnthropicTools() = map { with(it.function) { AnthropicAPI.ChatRequest.Tool(
    name, description, AnthropicAPI.ChatRequest.Parameters(parameters)
) } }

fun List<Message>.asAnthropicMessage() = this.map { AnthropicMessage(it) }

fun List<Message.Content>.asAnthropicContent(): List<AnthropicContent> = mapNotNull {
    when (it) {
        is Base64ImageContent -> Image(Image.ImageSource("base64", it.mediaType, it.base64))
        is TextContent -> Text(it.value)
        is DocumentContent -> null
    }
}

@Serializable
data class AnthropicMessage(val role: Role, val content: List<AnthropicContent>) {
    ...
    @JvmInline
    @SerialName("text")
    value class Text(val text: String) : AnthropicContent
    ...
    override suspend fun chat(tools: List<ToolConfig>): AnthropicChatResponse = with(self()) {
        return post(
            ...
            AnthropicAPI.ChatRequest(
                chatModel, messenger.messages().asAnthropicMessage(),
                maxTokens, temperature, tools.asAnthropicTools(), stream
            ),
            headers
        )
    }
